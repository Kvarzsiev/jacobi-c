...
void jacobi(int tam, int *matrizA, int *resultB, double *varsX, int np) {
  ...
  do {
    ...
    if (*contador == np) {
      ...
      *contador = 0;
      sem_post(semContador); // Release waiting processes
    }
    sem_post(mutex); // Exit the critical section

    sem_wait(semContador); // Wait until all processes reach the barrier
    sem_post(
        semContador); // Release the count semaphore for the next barrier usage
    // Barr ------
    printf("proc%d barrfim: \n\n", id);
